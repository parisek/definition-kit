...
- **A file is a page when its directory's parent is named `page`** — the same rule `parisek/styleguide` types an entry by. The `$schema` comment is an editor hint, not the rule.
