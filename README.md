...
- **Required:** `name`. **Allowed:** `usage`, `category`, `render`, `web`, `asana`, `figma`, `drupal`, `description`, `dev`, `aliases`, `variants_order`, `weight`, `responsive`, `body_class`, `variants` (legacy). **Refused:** `fields`, `kind`, `wp`, `key`, `mcp`.
...
- **Root metadata:** `name`, `category` and `fields` are required. `asana` is an absolute http(s) URL whose host is asana.com or a subdomain. `web` and `drupal` are a site-relative path (starts with `/`, not `//`) or an absolute http(s) URL with a host. Pages use the same formats for these keys. `aliases` is a list of extra search names for `parisek/styleguide`'s catalogue search: a plain string opens the entry itself, `{name, variant}` opens one variant tile (`variant` is a lowercase-kebab id). It has no CMS projection — it never reaches `acf.json` or `block.json` — so no generator or lint tool needs to know it; it lives in `<name>.yaml`, `page/<id>/<id>.yaml` and `doc/<id>/<id>.yaml` alike. Older `parisek/styleguide` versions ignore it. `variants_order` is a variant id, or a list of them, to show first in the catalogue; the rest keep file-name order. Same treatment: no CMS projection, shared across component, page and doc.
