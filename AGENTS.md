...
- `schemas/` — **shipped, runtime-read**: `component.fields.schema.json` (the `<name>.yaml` contract), `page.schema.json` (the `page/<id>/<id>.yaml` contract — a page is a separate type, detected by `Support\PageDefinition` from the nearest `page`/`component` ancestor directory), `acf-defaults-baseline.yaml` (type-defaults dropped on migrate / re-added on generate), `framework-props-baseline.yaml` (props the render pipeline injects into every component — implicitly declared everywhere, never written into a definition), `*.output.schema.json`, `constraint-sentinels.yaml`, `drift-lint-allowlist.yaml`, `block-icon.svg`. NOT export-ignored.
