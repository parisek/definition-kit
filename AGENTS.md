...
- `src/Drupal/` — the Drupal side: `DrupalConfig` (the lint's view of a `drush config:export` dir), `ConfigStore` (raw entities, for the generator), `DrupalSettings` (`drupal:` in definition-kit.yaml), `BundleResolver`, `DrupalTypeMap` (reads `schemas/drupal-type-map.yaml`), `DrupalBaseline`, `DrupalYaml` (dumps like `drush cex`), `DisplayEvidence` (token_get_all over the PHP display class). `Migration\DrupalParagraphReader` is config -> `fields:`.
- `src/Generator/Drupal/` — `fields-generate --target=drupal` (ADR 0002). `BundleSpecBuilder` (definition -> bundle/field specs, the lint's mapping reversed), `DrupalConfigPlanner` (merges owned keys into working copies of the export -> `Plan` of REUSE/CREATE/UPDATE/REFUSE), `DrupalConfigWriter`, `DrupalGenerator` (CLI orchestrator).
...
- `schemas/` — **shipped, runtime-read**: `component.fields.schema.json` (the `<name>.yaml` contract), `page.schema.json` and `doc.schema.json` (the `page/<id>/<id>.yaml` and `doc/<id>/<id>.yaml` contracts — separate types, detected by `Support\EntryDefinition` from the nearest `page`/`doc`/`component` ancestor directory; `Migration\EntryMetadataMigrator` moves their twig front-comment), `acf-defaults-baseline.yaml` (type-defaults dropped on migrate / re-added on generate), `drupal-type-map.yaml` + `drupal-defaults-baseline.yaml` (Drupal storage per kit type; what a created config entity gets), `framework-props-baseline.yaml` (props the render pipeline injects into every component — implicitly declared everywhere, never written into a definition), `*.output.schema.json`, `constraint-sentinels.yaml`, `drift-lint-allowlist.yaml`, `block-icon.svg`. NOT export-ignored.
...
- **Drupal generator: `generate(migrate(export))` plans no CREATE/UPDATE** against the same export (`DrupalGenerateRoundTripTest`). A key the generator owns must be one the migration reads; otherwise make it create-only (baseline on create, preserve on merge). Never delete; any REFUSE writes nothing.
